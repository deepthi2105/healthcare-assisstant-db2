...
db_ip = st.sidebar.text_input("Or enter IP address (optional)")
db_port = st.sidebar.text_input("Port", value="50000")
db_name = st.sidebar.text_input("Database Name")

if st.sidebar.button("Connect"):
    try:
        connection_host = db_ip if db_ip else db_host
        db_uri = f"db2+ibm_db://{db_user}:{db_pass}@{connection_host}:{db_port}/{db_name}"
